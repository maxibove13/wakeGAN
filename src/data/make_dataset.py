    ...
    for key, value in turns.items():
        if key in config["data"]["turns"]:
            ...
            p = multiprocessing.Process(
                target=make_dataset_per_prec,
                args=[prec, key, value, z, clim, plot_wf_slice, wt_xy],
            )
            # Start the process
            p.start()
...
def make_dataset_per_prec(prec, key, value, z, clim, plot_wf_slice, wt_xy):
    ...
    for u in config["data"]["fields"]:
        ...
        for comp, comp_names in enumerate(["Ux", "Uy"]):
            ...
            image, limits, grid_x, grid_y = get_image_from_simulation(
                case, z, t[prec], u, comp
            )
            ...
            im = axs[comp].imshow(
                ...
                origin="lower",
                extent=limits,
                vmin=clim_ux[0] if comp == 0 else clim_uy[0],
                vmax=clim_ux[1] if comp == 0 else clim_uy[1],
                interpolation="none",
            )
            ...
            wt_xy_sim = wt_xy[key]
            axs[comp].plot(
                wt_xy_sim[:, 0], wt_xy_sim[:, 1], "kx", markeredgewidth=1, markersize=4
            )
            ...
            for wt in range(wt_xy_sim.shape[0]):
                ...
                ids_wt, corners = isolate_wt_slice(
                    wt_xy_sim, wt, axs[comp], grid_x, grid_y, plot=plot_wf_slice
                )

                # obtain sample by interpolation
                x = np.linspace(corners[0][0], corners[1][0], config["data"]["size"][0])
                y = np.linspace(corners[2][1], corners[1][1], config["data"]["size"][1])
                X, Y = np.meshgrid(x, y)
                points = (np.flip(grid_y[:, 0]), grid_x[0, :])
                values = np.flip(image, axis=0)
                ...
                if comp == 0:
                    plt.imsave(
                        os.path.join(
                            "data", "preprocessed", "ux", f"{case}{wt}_ux.png"
                        ),
                        sample,
                        cmap=cm.gray,
                        vmin=config["data"]["figures"]["clim_ux"][0],
                        vmax=config["data"]["figures"]["clim_ux"][1],
                    )
                else:
                    plt.imsave(
                        os.path.join(
                            "data", "preprocessed", "uy", f"{case}{wt}_uy.png"
                        ),
                        sample,
                        cmap=cm.gray,
                        vmin=config["data"]["figures"]["clim_uy"][0],
                        vmax=config["data"]["figures"]["clim_uy"][1],
                    )
            ...
            # Set colorbar
            cbar = fig.colorbar(
                im, ax=axs[comp], orientation="vertical", aspect=10, pad=0.03
            )
        ...
        if plot_wf_slice:
            ...
            if u == "UMean":
                fig.suptitle(
                    f"Horizontal averaged velocity field at z ~ ${z}$ m\n${prec[0]}.{prec[-2:]}$ m/s | {key} = {value}° | {t[prec][0]} - {t[prec][1]}",
                    y=1.05,
                )
                fig.savefig(
                    os.path.join("figures", "wf_slices", f"um_{case}.png"),
                    facecolor="white",
                    transparent=False,
                    bbox_inches="tight",
                    dpi=300,
                ...
                fig.suptitle(
                    f"Horizontal instantaneous velocity field at z ~ ${z}$ m\n${prec[0]}.{prec[-2:]}$ m/s | {key} = {value}°",
                    y=1.05,
                )
                fig.savefig(
                    os.path.join("figures", "wf_slices", f"u_{case}.png"),
                    facecolor="white",
                    transparent=False,
                    bbox_inches="tight",
                    dpi=300,
...
def get_image_from_simulation(case, z, steps, field, comp):
    ...
    for reg_i in range(regions[0]):
        ...
        for reg_j in range(regions[1] - 1, -1, -1):
            ...
            U_slice, grid_x_slice, grid_y_slice, limits = get_horizontal_slice(
                case, z, steps, field, reg, regions[0] * regions[1], limits, comp
            )
            ...
            if reg_j == regions[1] - 1:
                ...
                image_col = np.concatenate((image_col, U_slice[1:, :]), axis=0)
                grid_x_col = np.concatenate((grid_x_col, grid_x_slice[1:, :]), axis=0)
                grid_y_col = np.concatenate((grid_y_col, grid_y_slice[1:, :]), axis=0)
        ...
        if reg_i == 0:
            ...
            image = np.concatenate((image, image_col[:, 1:]), axis=1)
            grid_x = np.concatenate((grid_x, grid_x_col[:, 1:]), axis=1)
            grid_y = np.concatenate((grid_y, grid_y_col[:, 1:]), axis=1)
...
def get_horizontal_slice(case, z, steps, field, reg, n_regions, limits, comp):
    ...
    if field == "UMean":
        U = (
            FL_1[reg].UMean * (steps[2] - steps[0])
            - FL_0[reg].UMean * (steps[1] - steps[0])
        ) / (steps[2] - steps[1])
    else:
        U = FL_1[reg].U
...
def isolate_wt_slice(wt_xy_sim, wt, ax, grid_x, grid_y, plot=True):
    ...
    lim_around_wt = config["data"][
        "lim_around_wt"
    ]  # left, right, top, bottom of the plane around the WT
    ...
    corners = [
        (
            wt_xy_sim[wt, 0] - lim_around_wt[0] * d,
            wt_xy_sim[wt, 1] + lim_around_wt[2] * d,
        ),  # Top-left
        (
            wt_xy_sim[wt, 0] + lim_around_wt[1] * d,
            wt_xy_sim[wt, 1] + lim_around_wt[2] * d,
        ),  # Top-right
        (
            wt_xy_sim[wt, 0] - lim_around_wt[0] * d,
            wt_xy_sim[wt, 1] - lim_around_wt[3] * d,
        ),  # Bottom-left
        (
            wt_xy_sim[wt, 0] + lim_around_wt[1] * d,
            wt_xy_sim[wt, 1] - lim_around_wt[3] * d,
...
if __name__ == "__main__":
    ...
    for prec in config["data"]["precs"]:
        make_dataset(
            config["data"]["z"],
            config["data"]["figures"]["plot_wf_slices"],
            wt_xy,
            clim,
        )
