...
def train():
    ...
    n_test_data = len(
        os.listdir(os.path.join("data", "preprocessed", "tracked", "test", "ux"))
    )
    print(
        f"\n"
    ...
    gen = Generator(
        CHANNELS, config["data"]["final_size"][0], config["models"]["f_g"]
    ).to(device)
    # Discriminator takes (C, f_d)
    disc = Discriminator(
        CHANNELS, config["models"]["f_d"], config["data"]["final_size"][1]
    ).to(device)
    print(
    ...
    opt_gen = torch.optim.Adam(
        gen.parameters(), lr=config["train"]["lr"], betas=(0.5, 0.999)
    )
    opt_disc = torch.optim.Adam(
        disc.parameters(), lr=config["train"]["lr"], betas=(0.5, 0.999)
    )
    ...
    if config["models"]["load"]:
        ...
        checkpoint_gen = torch.load(
            os.path.join("models", config["models"]["name_gen"]), map_location=device
        )
        ...
        checkpoint_disc = torch.load(
            os.path.join("models", config["models"]["name_disc"]), map_location=device
        )
    ...
    for fold, (train_idx, val_idx) in (
        enumerate(kfold.split(images)) if KFOLD else enumerate([(0, 0)])
    ):
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for image, inflow in trainloader if MULTIBATCH else [(0, 0)]:
                ...
                synths = gen(
                    inflow
                )  # gen generates flow field image from inflow velocity
                ...
                loss_g = criterion(
                    disc(synths, inflow), torch.ones_like(disc(synths, inflow))
                )  # Calculate loss
                ...
                for (im_tra, synth_tra) in zip(image, synths):
                    if CHANNELS == 1:
                        rmse_tra[0].append(
                            calc_mse(im_tra.detach(), synth_tra.detach())
                        )
                    elif CHANNELS == 2:
                        rmse_tra[0].append(
                            calc_mse(im_tra.detach()[0], synth_tra.detach()[0])
                        )
                        rmse_tra[1].append(
                            calc_mse(im_tra.detach()[1], synth_tra.detach()[1])
                        )
            ...
            if CHANNELS == 2:
                rmse_tra[1] = [
                    torch.sqrt(r).cpu().detach().numpy() for r in rmse_tra[1]
                ]
            ...
            for ims_test, inflow_test in testloader if MULTIBATCH else [(0, 0)]:
                ...
                with torch.no_grad():
                    ...
                    for synth_test, im_test in zip(synths_test, ims_test):
                        ...
                        if CHANNELS == 1:
                            rmse_test[0].append(
                                calc_mse(im_test.to(device), synth_test)
                            )
                        elif CHANNELS == 2:
                            rmse_test[0].append(
                                calc_mse(im_test.to(device)[0], synth_test[0])
                            )
                            rmse_test[1].append(
                                calc_mse(im_test.to(device)[1], synth_test[1])
                            )
            ...
            if CHANNELS == 2:
                rmse_test[1] = [
                    torch.sqrt(r).cpu().detach().numpy() for r in rmse_test[1]
                ]
            ...
            if epoch == 0:
                fig_im = plt.figure(
                    dpi=300
                )  # Initialize figure for flow field comparison
        ...
        if config["models"]["save"]:
            # Save Generator
            torch.save(
                {"state_dict": gen.state_dict(), "optimizer": opt_gen.state_dict()},
                os.path.join("models", config["models"]["name_gen"]),
            )
            # Save Discriminator
            torch.save(
                {"state_dict": disc.state_dict(), "optimizer": opt_disc.state_dict()},
